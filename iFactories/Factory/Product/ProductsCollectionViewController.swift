...
class ProductsCollectionViewController: UICollectionViewController {
    ...
    */
    ...
    override func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if let prd = myProducts[indexPath.row] as? Product

        {
            let url = URL(string: prd.image)
    ...
    */
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let vc = segue.destination as? ProductDetailsViewController {
            ...
            vc.product = myProducts[(self.collectionView?.indexPathsForSelectedItems![0].row)!] as? Product
        }
    }
}
